...
class FluidEnv(gym.Env):
    ...
    def step(self, action):
        ...
        Qxx = gaussian_filter ( np.asarray(states[0]).reshape(nx, ny)[::2, ::2], sigma = 1 ) + NOISE_STRENGTH*(0.5 - np.random.random([32, 32]))
        Qxy = gaussian_filter ( np.asarray(states[1]).reshape(nx, ny)[::2, ::2], sigma = 1 ) + NOISE_STRENGTH*(0.5 - np.random.random([32, 32]))
        ...
        uy_mid = uy[middle_start_col:middle_start_col + int((uy.shape[0]/50)*10), :]

        j_alpha = np.sum( (action_interpolated.reshape(64, 64) + 0.01*np.ones([64, 64]))**2 )
        ...
        .rl_module(
            rl_module_spec=SingleAgentRLModuleSpec(
                ...
                model_config_dict={
                    ...
                    "shared": False, 
                    "checkpoint": "/scratch1/saptorshighosh/ray_results/baseline_flow_new_nov9_noise_0.09/PPO_FluidEnv_4e59c_00000_0_2024-11-09_14-53-35/checkpoint_000106/policies/default_policy/policy_state.pkl",
                    "ds": False
                    }, 
            )
        )
    ...
    if args.no_tune:
        ...
        tuner = tune.Tuner(
            ...
            run_config=air.RunConfig(
                ...
                name = f"baseline_flow_new_nov9_noise_{NOISE_STRENGTH}"
            ),
        )
        results = tuner.fit()
