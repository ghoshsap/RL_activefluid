...
class FluidEnv(gym.Env):
    ...
    def reset(self, *, seed = None, options = None):
        ...
        Qxx = gaussian_filter ( Qxx_init[::2, ::2], sigma = 1 ) + NOISE_STRENGTH*(0.5 - np.random.random([32, 32])) 
        Qxy = gaussian_filter ( Qxy_init[::2, ::2], sigma = 1 ) + NOISE_STRENGTH*(0.5 - np.random.random([32, 32])) 
    ...
    def step(self, action):
        ...
        j_alpha = np.sum( (action_interpolated.reshape(64, 64) + 0.1*np.ones([64, 64]))**2 )
    ...
    if args.no_tune:
        ...
        tuner = tune.Tuner(
            ...
            run_config=air.RunConfig(
                ...
                name = f"noise_test_new_nov5_noise_{NOISE_STRENGTH}"
            ),
        )
        results = tuner.fit()
