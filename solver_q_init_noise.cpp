...
int main() {
    ...
    system.addNoise("Qxx", "0.04*q^2");
    system.addNoise("Qxy", "0.04*q^2");  
    ...
    while (true) {
        ...
        if (sum > NX*NY - 2)
        {
            ...
            size_t split_index = received_numbers.size()/4;  

            std::vector<float> Qxx_init(received_numbers.begin(), received_numbers.begin() + split_index);
            std::vector<float> Qxy_init(received_numbers.begin() + split_index, received_numbers.begin() + 2*split_index);
            std::vector<float> vx_init(received_numbers.begin() + 2*split_index, received_numbers.begin() + 3*split_index);
            std::vector<float> vy_init(received_numbers.begin() + 3*split_index, received_numbers.end());
            ...
            for (int j = 0; j < NY; j++)
            {
                ...
                {
                    int index = j * NX + i;
                    system.fields[0]->real_array[index].x =  Qxx_init[index];
                    system.fields[1]->real_array[index].x =  Qxy_init[index];
                    system.fields[9]->real_array[index].x =  vx_init[index];
                    system.fields[10]->real_array[index].x =  vy_init[index];
                }
            }
            ...
            cudaMemcpy(system.fields[9]->real_array_d, system.fields[9]->real_array, NX*NY*sizeof(float2), cudaMemcpyHostToDevice);
            system.fields[9]->toComp();
